...
class NodeFeatureBuilder:
    ...
    def default_vocab_sizes() -> Dict[str, int]:
        # Reasonable fallback sizes if not provided
        return {
            "node_type": len(ASTNodeType),
            "op": 1 + 1 + 32,  # pad+unk+ops approx
            "ctx": 5,          # <PAD>,<UNK>,Load,Store,Del
            "dtype": 6,        # <PAD>,<UNK>,bool,int,float,str
            "fn": 1 + 1 + 64,
            "attr": 1 + 1 + 32,
            "var": 128,
            "const_exact": 1 + 1 + 128,
