...
class ASTAutoencoder(nn.Module):
    ...
    def decode(
        ...
        for i in range(batch_size):
            # Generate program for this latent vector
            hidden = latent[i:i+1]  # Keep batch dimension
            ...
            try:
                ...
                generated_programs.append(result.get('program', ''))
                generation_info.append({
                    'success': result.get('success', False),
                    'steps': result.get('steps', 0),
                    'error': result.get('error'),
                    'partial_program': result.get('partial_program', ''),
                    'productions_used': result.get('productions_used', [])
                })
